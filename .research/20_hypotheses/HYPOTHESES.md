...
**Notes:** This is a placeholder hypothesis serving as a worked example of the entry format. The 2x threshold is a working-hypothesis effect size and may be revised once Sprint 0 Week 1 yields the empirical variance distribution; the revision (if any) must happen BEFORE F is chosen, to avoid post-hoc threshold tuning. F is now scoped to the top-3 ranked candidates in `20_hypotheses/FOCUS_SHORTLIST.md` (drafted 2026-05-18 from run-01 data signals + `10_literature/focus_candidates/` briefs): F in {ribozyme, riboswitch, viral_rna}. The user picks one at end of Week 1.
**Refinement plan:** Once user picks F, fork H-001 into H-001a (focus-specific), draft H-001a's protocol at `protocols/H-001a.md`, then queue rna-code's first real inference run for the focus family.
